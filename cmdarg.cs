...
namespace xayrga
{
    public static class cmdarg
    {
        ...
        public static int assertArgNum(int argn, string assert)
        {
            ...
            int b = 1;
            var w = Int32.TryParse(cmdargs[argn], System.Globalization.NumberStyles.AllowHexSpecifier, null, out b);
            if (w == false)
            {
        ...
        public static uint assertArgNumUI(int argn, string assert)
        {
            if (cmdargs.Length <= argn)
            {
                Console.WriteLine("Missing required argument #{0} for '{1}'", argn, assert);
            ...
            uint b = 1;
            var w = UInt32.TryParse(cmdargs[argn], System.Globalization.NumberStyles.AllowHexSpecifier, null, out b);
            if (w == false)
            {
                Console.WriteLine("Cannot parse argument #{0} for '{1}' (expected number, got {2}) ", argn, assert, cmdargs[argn]);
                Environment.Exit(0);
